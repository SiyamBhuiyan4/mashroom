...
import { User, LogOut, Settings, BarChart3, Package, Coins, LayoutGrid, Camera, X, MessageSquare } from 'lucide-react';
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)', background: 'rgba(7,26,14,0.6)', backdropFilter: 'blur(8px)', overflowX: 'auto' }}>
        {TAB.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1rem 1.5rem', background: 'transparent', border: 'none', color: tab === t.id ? 'var(--color-primary)' : 'var(--text-muted)', cursor: 'pointer', borderBottom: tab === t.id ? '2px solid var(--color-primary)' : '2px solid transparent', transition: 'all 0.2s', fontFamily: 'var(--font-body)', fontSize: '0.9rem', whiteSpace: 'nowrap', position: 'relative' }}>
            {t.icon} {t.label}
            {t.id === 'orders' && pendingOrders.length > 0 && (
              <span style={{ position: 'absolute', top: '8px', right: '8px', background: 'var(--color-warning)', color: '#000', borderRadius: '50%', width: '16px', height: '16px', fontSize: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>
